...
| External provider account identity | Future provider installation; foundation operations/tests | Resolve Platform Connection and route provider traffic | Yes: connection UUID, Tenant UUID, provider, external account ID, creation timestamp | PostgreSQL `platform_connection` | TBD | Cascades with Tenant-row deletion; disconnect workflow TBD | No production subprocessor selected | Implemented foundation identity only |
